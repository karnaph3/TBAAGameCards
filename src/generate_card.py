...
class GameCardApp:
    ...
    def show_dropdowns(self):
        ...
        for idx, field in enumerate(REQUIRED_FIELDS):
            Label(self.mapping_frame, text=field).grid(
            ...
            for i, row in self.df.iterrows():
                try:
                    data = {
                        ...
                        "Game_Number": str(row[mapping["Game_Number"]]).strip(),
                        "Age": str(row[mapping["Age"]]).strip(),
                        "Date": str(row[mapping["Date"]]).strip(),
                        "Field_Number": str(row[mapping["Field_Number"]]).strip(),
                        "Start_Time": str(row[mapping["Start_Time"]]).strip(),
                        "Home_Team": str(row[mapping["Home_Team"]]).strip(),
                        "Away_Team": str(row[mapping["Away_Team"]]).strip(),
                        "Referee": str(row[mapping["Referee"]]).strip(),
                        "Linesman1": str(row[mapping["Linesman1"]]).strip(),
                        "Linesman2": str(row[mapping["Linesman2"]]).strip(),
                    }
